...
function Tobii(userOptions) {
  ...
  var supportedElements = {
    image: {
      checkSupport: function checkSupport(el) {
        return (
          ...
          el.href.match(/\.(png|jpe?g|tiff|tif|gif|bmp|webp|svg|ico)(\?.*)?$/i)
        )
      },
    ...
    html: {
      ...
      init: function init(el, container) {
        var targetSelector = el.hasAttribute('href')
  ...
  var close = function close(callback) {
    ...
    var container =
      groups[activeGroup].sliderElements[groups[activeGroup].currentIndex].querySelector(
        '[data-type]',
      )
  ...
  var updateCounter = function updateCounter() {
    counter.textContent =
      groups[activeGroup].currentIndex + 1 + '/' + groups[activeGroup].elementsLength
  }
  /**
  ...
  var updateFocus = function updateFocus(dir) {
    ...
    lastFocusableEl =
      focusableEls.length === 1 ? focusableEls[0] : focusableEls[focusableEls.length - 1]
  }
  /**
  ...
  var updateAfterDrag = function updateAfterDrag() {
    ...
    if (
      movementX > 0 &&
      movementXDistance > config.threshold &&
  ...
  var clickHandler = function clickHandler(event) {
    if (event.target === prevButton) {
      ...
      (event.target.className === 'tobii__slider-slide' && config.docClose)
    ) {
      close()
    }
